...
# which results in the occasional reduction of MFMM log-likelihood calculations.
lmult <- function( x, p)  {
...
#oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
MFMM <- function(dat, K, multinomial_indices = integer(), tol = 0.01, est_cov = TRUE)  {
  ...
  while( eps > 0.01)	{
    ...
    if( len_num>0  )	{
      ...
      if( est_cov )	{
        ...
        sig_sq_k <- mapply(function(y,z) t(y)*z / sum(z), XmMU_sq_ik, Z ,SIMPLIFY = FALSE)
        sig_sq_k <- lapply(sig_sq_k,function(x) apply(x,2,sum))
        sig_sq_k <- lapply(sig_sq_k, function(x) matrix(x, nrow=len_num,ncol=len_num))
      } else {
        #est - gaussian 0 correlation covariance matrix
        XmMU_ik <- lapply(mu_k, function(x) t(t(X_n) - x) )
        XmMU_sq_ik <- lapply(XmMU_ik, '^', 2)
        sig_sq_k <- mapply(function(y,z) y*z / sum(z), XmMU_sq_ik, Z , SIMPLIFY = FALSE)
        sig_sq_k <- lapply(sig_sq_k,function(x) apply(x,2,sum))
        sig_sq_k <- lapply(sig_sq_k, diag)
      }
    }
...
# To prevent printing of all data when MFMM output is called
print.mfmm <- function (x) {
  hid <- attr(x, "hidden")
